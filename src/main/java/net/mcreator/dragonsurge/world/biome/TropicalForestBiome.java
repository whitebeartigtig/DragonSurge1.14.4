...
public class TropicalForestBiome extends DragonSurgeElements.ModElement {
	@ObjectHolder("dragonsurge:tropicalforest")
	public static final CustomBiome biome = null;
	public TropicalForestBiome(DragonSurgeElements instance) {
		super(instance, 21);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(1f).depth(0.2f).scale(0.3f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-13421569).waterFogColor(-13421569)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.GRASS_BLOCK.getDefaultState(),
							Blocks.IRON_ORE.getDefaultState(), Blocks.IRON_ORE.getDefaultState())));
			setRegistryName("tropicalforest");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Biome.createDecoratedFeature(new CustomTreeFeature(),
					IFeatureConfig.NO_FEATURE_CONFIG, Placement.COUNT_EXTRA_HEIGHTMAP, new AtSurfaceWithExtraConfig(14, 0.1F, 1)));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.GRASS_BLOCK.getDefaultState().getBlock() || ground == Blocks.IRON_ORE.getDefaultState().getBlock())
							&& (ground2 == Blocks.GRASS_BLOCK.getDefaultState().getBlock()
									|| ground2 == Blocks.IRON_ORE.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Blocks.IRON_ORE.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == Blocks.JUNGLE_LOG.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, Blocks.JUNGLE_LOG.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = position.getY() - 3 + height; genh <= position.getY() + height; genh++) {
							...
							for (int genx = position.getX() - k4; genx <= position.getX() + k4; genx++) {
								for (int genz = position.getZ() - k4; genz <= position.getZ() + k4; genz++) {
									...
									if (state.isIn(BlockTags.LEAVES) || state.getBlock() == Blocks.JUNGLE_LOG.getDefaultState().getBlock()) {
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.JUNGLE_LOG.getDefaultState().getBlock()
					|| blockType == Blocks.JUNGLE_LOG.getDefaultState().getBlock() || blockType == Blocks.GRASS_BLOCK.getDefaultState().getBlock()
					|| blockType == Blocks.IRON_ORE.getDefaultState().getBlock();
		}
