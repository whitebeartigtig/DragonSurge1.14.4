...
import net.mcreator.dragonsurge.block.LightDragonOreBlock;
...
public class LightDragonForestJungleBiome extends DragonSurgeElements.ModElement {
	@ObjectHolder("dragonsurge:lightdragonforestjungle")
	public static final CustomBiome biome = null;
	public LightDragonForestJungleBiome(DragonSurgeElements instance) {
		super(instance, 140);
	}
		...
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(LightdragonsurgegrassBlock.block.getDefaultState(),
							LightDragonOreBlock.block.getDefaultState(), LightDragonOreBlock.block.getDefaultState())));
			setRegistryName("lightdragonforestjungle");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == LightdragonsurgegrassBlock.block.getDefaultState().getBlock()
							|| ground == LightDragonOreBlock.block.getDefaultState().getBlock())
							&& (ground2 == LightdragonsurgegrassBlock.block.getDefaultState().getBlock()
									|| ground2 == LightDragonOreBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), LightDragonOreBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.JUNGLE_LOG.getDefaultState().getBlock()
					|| blockType == Blocks.JUNGLE_LOG.getDefaultState().getBlock()
					|| blockType == LightdragonsurgegrassBlock.block.getDefaultState().getBlock()
					|| blockType == LightDragonOreBlock.block.getDefaultState().getBlock();
		}
