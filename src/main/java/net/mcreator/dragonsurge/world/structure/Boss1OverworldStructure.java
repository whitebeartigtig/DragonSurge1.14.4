...
public class Boss1OverworldStructure extends DragonSurgeElements.ModElement {
	public Boss1OverworldStructure(DragonSurgeElements instance) {
		super(instance, 164);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 1) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("dragonsurge", "boss13"));
						if (template == null)
							return false;
