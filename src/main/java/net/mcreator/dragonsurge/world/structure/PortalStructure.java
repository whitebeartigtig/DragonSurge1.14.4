...
package net.mcreator.dragonsurge.world.structure;
...
import net.mcreator.dragonsurge.world.dimension.LightDragonSurgeDimension;
import net.mcreator.dragonsurge.world.dimension.DarkDragonSurgeDimension;
import net.mcreator.dragonsurge.DragonSurgeElements;

import java.util.Random;

@DragonSurgeElements.ModElement.Tag
public class PortalStructure extends DragonSurgeElements.ModElement {
	public PortalStructure(DragonSurgeElements instance) {
		super(instance, 134);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize) {
			...
			public boolean place(IWorld iworld, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if (dimensionType == LightDragonSurgeDimension.type)
					dimensionCriteria = true;
				if (dimensionType == DarkDragonSurgeDimension.type)
					dimensionCriteria = true;
				...
				if ((random.nextInt(1000000) + 1) <= 100) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("dragonsurge", "darkdragonsurgeportal"));
						if (template == null)
							return false;
